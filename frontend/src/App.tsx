...
const navItems = [
  ...
  { label: "运营分析", icon: LineChart },
  { label: "团队分析", icon: Users },
  { label: "用户分析", icon: Box },
  { label: "成本分析", icon: WalletCards },
  { label: "代码入库分析", icon: Database },
    ...
    <div className="app-shell">
      <aside className="sidebar">
        ...
        <nav className="nav-list">
          {navItems.map((item) => (
            <button key={item.label} className={`nav-item ${item.active ? "is-active" : ""}`}>
              <item.icon size={18} />
        ...
        <div className="sync-time">
          <RefreshCcw size={14} />
          <span>数据更新时间</span>
          <strong>{overview?.updated_at ?? "2025-05-20 10:30"}</strong>
        </div>
      </aside>
      ...
      <main className="dashboard">
        ...
        <section className="filter-panel">
          {filterOptions ? (
            <FilterGrid options={filterOptions} filters={activeFilters} onChange={setFilters} onQuery={() => query(filters)} />
          ) : (
...
function FilterGrid({
  ...
  return (
    <div className="filter-grid">
      ...
      <button className="query-button" onClick={onQuery}>查询</button>
    </div>
  );
}
    ...
    <article className={`kpi-card accent-${metric.accent}`}>
      ...
      <div>
        ...
        <p className={`delta ${metric.direction}`}>
          ...
          <span>{metric.direction === "down" ? "▼" : "▲"} {metric.change}</span>
        </p>
      </div>
    </article>
  );
...
function ChartPanel({ title, action, children }: { title: string; action?: string; children: ReactNode }) {
  return (
    <article className="panel">
      <div className="panel-header">
        <h2>{title}<span className="info-dot">i</span></h2>
        {action ? <button className="small-select">{action}<ChevronDown size={14} /></button> : null}
      </div>
      {children}
    </article>
  );
...
function RankingPanel({ rows }: { rows: RankingRow[] }) {
  return (
    <article className="panel ranking-panel">
      <div className="panel-header">
        <h2>PDU / LM 团队运营排行<span className="info-dot">i</span></h2>
      </div>
...
function InsightsPanel({ insights }: { insights: Insight[] }) {
  return (
    <article className="panel insights-panel">
      <div className="panel-header">
        <h2>重点运营关注<span className="info-dot">i</span></h2>
      </div>
      <div className="insight-list">
        {insights.map((item) => (
          <div key={item.title} className={`insight ${item.type}`}>
            <div className="insight-icon">{item.type === "risk" ? "!" : item.type === "warning" ? "?" : item.type === "success" ? "↗" : "i"}</div>
            <div>
...
function DetailTabs({ users, mrs, tokens }: { users: UserDetail[]; mrs: MrDetail[]; tokens: TokenDetail[] }) {
...
function quadrantOption(data: DashboardOverview) {
  return {
    ...
    tooltip: {
      formatter: (params: { data: [number, number, number, string] }) => `${params.data[3]}<br/>Token: ${params.data[0]}亿<br/>AI MR占比: ${params.data[1]}%`
    },
    ...
    series: [
      {
        ...
        itemStyle: {
          color: (params: { dataIndex: number }) => ["#10b99a", "#17a48d", "#2c73f6", "#1f8cff", "#ef3445"][params.dataIndex % 5]
        },
        label: {
          show: true,
